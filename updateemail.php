...
if (!filter_var($newemail,FILTER_VALIDATE_EMAIL)) {
  echo "<div class='alert alert-danger'>Enter a valid email-address</div>";exit;
}
...
$message .= "http://onlinenotes.sunilhosting.online/activatenewemail.php?email=" . urlencode($email) . "&newemail=" . urlencode($newemail) . "&key=$activationKey";
if(mail($newemail, 'Email Update for you Online Notes App', $message, 'From:'.'UpdateEmail@onlinenotes.com')){
       echo "<div class='alert alert-success'>An email has been sent to <strong>$newemail</strong>. Please click on the link to prove you own that email address.</div>";
}
